...
// Cache Teks Hasil AI untuk Fitur Salin
let textRawHasilAiDalil = '';

// Groq API Key Configuration
const GROQ_API_KEY = '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';
...
function bersihkanTeksHtml(str) {
    if (!str) return '';
...
window.cariDalilAiGroq = async function() {
    ...
    const rawTema = inputTema.value.trim();
    // Ganti underscore (_) dengan spasi jika user mengetik menggunakan underscore
    const tema = rawTema.replace(/_/g, ' ');
    ...
    // Menggunakan prompt tag token yang kebal terhadap kegagalan parsing JSON
    const systemPrompt = `Anda adalah pakar Al-Qur'an dan Hadis Sahih.
Tugas Anda memberikan 1-2 Ayat Al-Qur'an dan 1-2 Hadits Sahih sesuai tema materi pembelajaran.

Gunakan format TAG PERSIS seperti di bawah ini tanpa teks pembuka/penutup dan tanpa tag HTML:

[AYAT]
Surah: (Nama Surah dan Nomor Ayat, cth: QS. Al-Baqarah : 183)
Arab: (Teks Arab berharakat lengkap)
Arti: (Terjemahan bahasa Indonesia)
[/AYAT]

[HADIS]
Perawi: (Nama Perawi Sahih, cth: HR. Bukhari no. 1 atau HR. Muslim)
Arab: (Teks Arab berharakat lengkap)
Arti: (Terjemahan bahasa Indonesia)
[/HADIS]`;
    ...
    try {
        const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    ...
                    { role: 'user', content: `Carikan dalil untuk tema: "${tema}"` }
                ],
                temperature: 0.2
            })
        });
        ...
        // Parsing Blok Ayat
        const ayatMatches = [...rawContent.matchAll(/\[AYAT\]([\s\S]*?)\[\/AYAT\]/gi)];
        // Parsing Blok Hadis
        const hadisMatches = [...rawContent.matchAll(/\[HADIS\]([\s\S]*?)\[\/HADIS\]/gi)];

        // 1. Render Ayat Al-Qur'an
        if (ayatMatches.length > 0) {
            ...
            ayatMatches.forEach((m, idx) => {
                const block = m[1];
                const surah = bersihkanTeksHtml((block.match(/Surah:\s*(.*)/i) || [])[1] || 'Al-Qur\'an');
                const arab = bersihkanTeksHtml((block.match(/Arab:\s*([\s\S]*?)(?=Arti:|$)/i) || [])[1] || '-');
                const arti = bersihkanTeksHtml((block.match(/Arti:\s*([\s\S]*)/i) || [])[1] || '-');
                ...
                `;
                textSalinBuilder += `${idx + 1}. ${surah}\n${arab}\nArtinya: "${arti}"\n\n`;
            });
        }

        // 2. Render Hadis Sahih
        if (hadisMatches.length > 0) {
            ...
            hadisMatches.forEach((m, idx) => {
                const block = m[1];
                const perawi = bersihkanTeksHtml((block.match(/Perawi:\s*(.*)/i) || [])[1] || 'Hadits Sahih');
                const arab = bersihkanTeksHtml((block.match(/Arab:\s*([\s\S]*?)(?=Arti:|$)/i) || [])[1] || '-');
                const arti = bersihkanTeksHtml((block.match(/Arti:\s*([\s\S]*)/i) || [])[1] || '-');
                ...
                `;
                textSalinBuilder += `${idx + 1}. HR. ${perawi}\n${arab}\nArtinya: "${arti}"\n\n`;
            });
        }

        // Jika model mengembalikan format bebas di luar tag
        if (ayatMatches.length === 0 && hadisMatches.length === 0) {
            const cleanRaw = bersihkanTeksHtml(rawContent);
            html = `
                <div class="dalil-item-card" style="font-size:11px; line-height:1.6; color:var(--text-putih); white-space:pre-wrap;">
                    ${cleanRaw}
                </div>
            `;
            textSalinBuilder += cleanRaw;
        }

        textRawHasilAiDalil = textSalinBuilder.trim();
...
window.copySemuaDalilText = async function() {
    if (!textRawHasilAiDalil) {
        alert("Belum ada hasil dalil yang dapat disalin!");
        return;
    }

    const teksMurni = bersihkanTeksHtml(textRawHasilAiDalil);

    try {
        await navigator.clipboard.writeText(teksMurni);
        alert("Teks dalil murni berhasil disalin ke clipboard!");
    } catch (err) {
        const tempInput = document.createElement("textarea");
        tempInput.value = teksMurni;
        document.body.appendChild(tempInput);
        tempInput.select();
        document.execCommand("copy");
        document.body.removeChild(tempInput);
        alert("Teks dalil murni berhasil disalin!");
    }
};

// ================= 8. FITUR WHATSAPP GROUP (TERSEMBUNYI DEFAULT) =================
